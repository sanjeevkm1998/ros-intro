...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<simRosAuxiliaryConsoleOpen-request>)))
  ...
  (cl:format cl:nil "#~%# simInt simAuxiliaryConsoleOpen(const simChar* title,simInt maxLines,simInt mode,simInt* position,simInt* size,simFloat* textColor,simFloat* backgroundColor)~%#~%~%string title~%int32 maxLines~%int32 mode~%int32[] position~%int32[] size~%float32[] textColor~%float32[] backgroundColor~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'simRosAuxiliaryConsoleOpen-request)))
  "Returns full string definition for message of type 'simRosAuxiliaryConsoleOpen-request"
  (cl:format cl:nil "#~%# simInt simAuxiliaryConsoleOpen(const simChar* title,simInt maxLines,simInt mode,simInt* position,simInt* size,simFloat* textColor,simFloat* backgroundColor)~%#~%~%string title~%int32 maxLines~%int32 mode~%int32[] position~%int32[] size~%float32[] textColor~%float32[] backgroundColor~%~%~%"))
